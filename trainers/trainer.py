    ...
    def train(self, model, d_dataloaders, logger=None, logdir=""):
        ...
        for i_epoch in range(1, cfg['n_epoch'] + 1):
            for x, _ in train_loader:
                ...
                if (cfg.eval_interval is not None) and (i_iter % cfg.eval_interval == 0):  
                    d_eval = model.eval_step(val_loader, device=self.device, **cfg)
                    ...
                    eval_loss = d_eval[cfg['besteval_key']]
                    best_eval_model = eval_loss < best_eval_loss
                    
                    if best_eval_model:
                        print(f'Iter [{i_iter:d}] best eval model saved {eval_loss:.6f} <= {best_eval_loss:.6f}')
                        best_eval_loss = eval_loss
                        self.save_model(model, logdir, besteval=True)

                if (cfg.visualize_interval is not None) and (i_iter % cfg.visualize_interval) == 0:
                    d_val = model.visualization_step(val_loader, device=self.device)
                    logger.add_val(i_iter, d_val)

        self.save_model(model, logdir, i_iter="last")
        return model, best_val_loss

    def save_model(self, model, logdir, best=False, besteval=False, i_iter=None, i_epoch=None):
        if best:
            pkl_name = "model_best.pkl"
        elif besteval:
            pkl_name = "model_besteval.pkl"
        else:
